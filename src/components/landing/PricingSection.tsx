    ...
    <section ref={ref} className="section-padding">
      <div className="container mx-auto max-w-6xl">
        <div className={`text-center max-w-2xl mx-auto mb-16 ${isVisible ? "animate-fade-up" : "opacity-0"}`}>
          ...
          <p className="text-muted-foreground text-lg mt-4">Start free. Scale as you grow.</p>
        </div>
        <div className="grid md:grid-cols-3 gap-6">
          {plans.map((plan, i) => (
            <div key={plan.name} className={`relative rounded-2xl p-8 transition-all hover:-translate-y-1 ${plan.highlighted ? "gradient-bg text-white shadow-2xl shadow-solar-orange/20 scale-105 border-0" : "border border-border bg-card hover:shadow-xl hover:shadow-solar-orange/10"} ${isVisible ? "animate-fade-up" : "opacity-0"}`} style={{ animationDelay: `${i * 0.1}s` }}>
              {plan.highlighted && (
                <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-white text-solar-orange text-xs font-bold px-4 py-1 rounded-full shadow-lg">Most Popular</div>
              )}
              ...
              <Button className={`w-full mb-6 ${plan.highlighted ? "bg-white text-solar-orange hover:bg-white/90 font-bold" : "gradient-bg text-white border-0"}`} size="lg">
                {plan.name === "Enterprise" ? "Contact Sales" : "Start Free Trial"}
              </Button>
              <div className="space-y-3">
                {plan.features.map((f) => (
                  <div key={f} className="flex items-center gap-2 text-sm">
                    <Check className={`w-4 h-4 flex-shrink-0 ${plan.highlighted ? "text-white" : "text-solar-orange"}`} />
